...
import { LayoutDashboard, CalendarPlus, ListMusic, Ticket, LogOut, User, Settings, Home } from 'lucide-react';
        ...
        <header className="flex items-center justify-between px-8 py-4 bg-white border-b border-gray-200">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">
              {location.pathname === '/organizer/dashboard' && 'Tableau de Bord'}
              {location.pathname === '/organizer/my-events' && 'Mes Événements'}
              {location.pathname === '/organizer/create-event' && 'Créer un Événement'}
              {location.pathname === '/organizer/tickets' && 'Gestion des Tickets'}
              {location.pathname.includes('/organizer/edit-event') && 'Modifier l\'Événement'}
            </h2>
            <p className="text-sm text-gray-500">Espace organisateur</p>
          </div>
              ...
              <button className="flex items-center gap-3 px-3 py-2 transition-colors rounded-lg hover:bg-gray-50">
                <Avatar className="w-10 h-10 border-2 border-green-600">
                  ...
                  <AvatarFallback className="font-semibold text-green-700 bg-green-100">
                    {user?.prenom?.charAt(0) || user?.nom?.charAt(0) || 'O'}
                  </AvatarFallback>
                </Avatar>
